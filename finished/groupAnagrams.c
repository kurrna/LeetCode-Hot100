    ...
    *returnSize = 0;
    ...
    for (int i = 0; i < strsSize; i++)
    {
        ...
        for (int j = i + 1; j < strsSize; j++)
        {
            ...
            {
                len = strlen(*(strs + j)) + 1;
                ret[*returnSize][(*returnColumnSizes)[*returnSize]] = (char *)malloc(len * sizeof(char));
                strcpy(ret[*returnSize][(*returnColumnSizes)[*returnSize]], *(strs + j));
